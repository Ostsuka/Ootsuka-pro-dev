...
const PROJECTS: Project[] = [
  {
    title: 'EC 一元管理 SaaS — 商品・在庫同期',
    titleEn: 'EC Unified Management SaaS',
    ...
    metrics: [
      { label: 'エラー率削減', value: '20%↓', color: '#10b981' },
      { label: 'チーム規模',   value: '5名',   color: '#00c8f0' },
      { label: '担当フェーズ', value: '設計〜運用', color: '#8b5cf6' },
    ],
    accentColor: '#00c8f0',
    icon: <Code2 size={22} />,
  },
  {
    title: 'クラウド対応 業務 Web システム',
    titleEn: 'Cloud-Native Business Web App',
    ...
    metrics: [
      { label: '生産性向上',   value: '30%↑',  color: '#10b981' },
      { label: '開発期間',     value: '29ヶ月', color: '#00c8f0' },
      { label: 'チーム規模',   value: '5〜10名', color: '#8b5cf6' },
    ],
    accentColor: '#8b5cf6',
    icon: <Cloud size={22} />,
  },
  {
    title: '写真・点検帳票管理システム',
    titleEn: 'Photo & Inspection Form Management',
    category: '製造・流通業向け',
    ...
    metrics: [
      { label: 'チーム規模',  value: '25名',    color: '#f59e0b' },
      { label: '継続期間',    value: '22ヶ月',  color: '#00c8f0' },
      { label: 'DB 最適化',   value: '高速化',  color: '#10b981' },
    ],
    accentColor: '#10b981',
    icon: <Database size={22} />,
  },
  {
    title: '卸売商品管理システム AWS 移行',
    titleEn: 'Wholesale System AWS Migration',
    ...
    metrics: [
      { label: '移行先',    value: 'AWS ECS',    color: '#f59e0b' },
      { label: '監視体制',  value: 'CloudWatch', color: '#00c8f0' },
      { label: 'チーム規模', value: '3〜5名',    color: '#8b5cf6' },
    ],
    accentColor: '#f59e0b',
...
function ProjectCard({ proj, index }: { proj: Project; index: number }) {
  ...
  return (
    ...
    >
      {/* Top accent line */}
      <div className="h-[2px]" style={{ background: `linear-gradient(90deg, ${ac}, transparent)` }} />
      ...
      <div className="p-6 flex flex-col flex-1">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <span
            ...
            style={{ background: `${ac}10`, color: ac, border: `1px solid ${ac}28` }}
          >
            {proj.category}
          </span>
        </div>

        <h3 className="text-base font-bold mb-0.5 leading-snug" style={{ color: '#f1f5f9' }}>{proj.title}</h3>
        <p className="text-[11px] mb-3" style={{ color: '#334155' }}>{proj.titleEn}</p>
        <p className="text-sm leading-7 mb-5 flex-1" style={{ color: '#94a3b8' }}>{proj.description}</p>
        ...
        <div className="grid grid-cols-3 gap-2 mb-5">
          {proj.metrics.map(m => (
            ...
            >
              ...
              <div className="text-[10px] mt-0.5" style={{ color: '#334155' }}>{m.label}</div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-1.5 mb-4">
          {proj.stack.slice(0, 5).map(s => (
            <span key={s} className="tag-neutral" style={{ fontSize: '0.67rem' }}>{s}</span>
          ))}
          {proj.stack.length > 5 && (
            <span className="tag-neutral" style={{ fontSize: '0.67rem' }}>+{proj.stack.length - 5}</span>
          )}
        </div>
        ...
        <AnimatePresence initial={false}>
          {expanded && (
            ...
            >
              <div className="mt-4 space-y-3 pt-4" style={{ borderTop: `1px solid ${ac}12` }}>
                {[
                  { label: '課題',   text: proj.challenge, color: '#f59e0b' },
                  { label: '解決策', text: proj.solution,  color: ac },
                  { label: '成果',   text: proj.result,    color: '#10b981' },
                ].map(item => (
                  <div key={item.label} className="flex gap-3">
                    <span
                      className="text-[10px] font-bold px-1.5 py-0.5 rounded flex-shrink-0 h-fit mt-0.5"
                      style={{ background: `${item.color}15`, color: item.color }}
                    >
                      {item.label}
                    </span>
                    <p className="text-xs leading-6" style={{ color: '#94a3b8' }}>{item.text}</p>
                  </div>
                ))}
    ...
    <section id="projects" className="section relative" style={{ background: '#050810' }}>
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <p className="mt-6 text-sm max-w-lg mx-auto" style={{ color: '#64748b' }}>
