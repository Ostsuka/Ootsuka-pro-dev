...
const BADGES = [
  { label: 'React',       color: '#61dafb', x: '4%',  y: '22%', delay: 0.0 },
  { label: 'TypeScript',  color: '#3178c6', x: '80%', y: '18%', delay: 0.6 },
  { label: 'Java',        color: '#f89820', x: '2%',  y: '60%', delay: 1.1 },
  { label: 'AWS',         color: '#ff9900', x: '84%', y: '58%', delay: 1.6 },
  { label: 'NestJS',      color: '#e0234e', x: '74%', y: '36%', delay: 0.9 },
  { label: 'Docker',      color: '#2496ed', x: '10%', y: '42%', delay: 1.4 },
  { label: 'Laravel',     color: '#ff2d20', x: '44%', y: '87%', delay: 1.9 },
  { label: 'PostgreSQL',  color: '#336791', x: '18%', y: '78%', delay: 2.3 },
];

/* ── Trust / Value propositions (rotating) ──────── */
const VALUE_PROPS = [
  { icon: <TrendingUp size={15} />, text: '売上・問い合わせにつながるシステムを設計・構築します', color: '#10b981' },
  { icon: <Zap size={15} />,        text: '業務プロセスの非効率を、テクノロジーで解決します',    color: '#00d4ff' },
  { icon: <Users size={15} />,      text: '要件定義から運用まで一人でフルサイクル対応可能です',  color: '#a78bfa' },
];

/* ── Hero metrics ────────────────────────────────── */
const METRICS = [
  { value: '5+',   label: '年の実務経験',   sub: 'Years Exp.',    color: '#00d4ff' },
  { value: '30%↑', label: '業務効率化実績', sub: 'Productivity',  color: '#10b981' },
  { value: '20%↓', label: 'エラー率削減',   sub: 'Error Reduce',  color: '#a78bfa' },
  { value: '4社',  label: '継続取引実績',   sub: 'Long-term',     color: '#f59e0b' },
];

/* ── Trust badges row ────────────────────────────── */
const TRUST_BADGES = [
  { text: '即日対応可',        color: '#10b981', icon: <CheckCircle2 size={12} /> },
  { text: '日本語で仕様確認',  color: '#00d4ff', icon: <CheckCircle2 size={12} /> },
  { text: '要件定義〜運用まで一貫', color: '#a78bfa', icon: <CheckCircle2 size={12} /> },
  { text: '現在受付中',        color: '#f59e0b', icon: <CheckCircle2 size={12} /> },
];

/* ── Social proof mini strip ─────────────────────── */
const SOCIAL_PROOF = [
  { icon: <Star size={12} fill="#f59e0b" style={{ color: '#f59e0b' }} />, text: '評価 5.0 / 5.0' },
  { icon: <Shield size={12} style={{ color: '#10b981' }} />,              text: '100% リピート意向' },
  { icon: <Clock size={12} style={{ color: '#00d4ff' }} />,              text: '24h 以内返信' },
];
...
const CODE_LINES = [
  ...
  { text: '  efficiency: "+30%",',       color: '#10b981' },
  { text: '  available:  true,',         color: '#00d4ff' },
  { text: '  languages:  ["日本語","中文","EN"],', color: '#a78bfa' },
  { text: '};',                          color: '#e2e8f0' },
];
    ...
    >
      ...
      >
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
          ...
          <div className="rounded-2xl px-5 py-3 flex items-center gap-3 justify-center"
            style={{ background: 'rgba(0,212,255,0.05)', border: '1px solid rgba(0,212,255,0.15)', backdropFilter: 'blur(12px)', minHeight: '48px' }}>
          ...
          <div className="flex justify-center gap-1.5 mt-2">
            {VALUE_PROPS.map((_, i) => (
              <button key={i} onClick={() => setActiveValue(i)} className="rounded-full transition-all"
                style={{ width: activeValue === i ? '20px' : '6px', height: '6px',
        ...
        {/* Name */}
        <motion.h1 initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.75 }}
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.72 }}
          ...
          {METRICS.map((m, i) => (
            <motion.div key={m.label}
              ...
              style={{ background: `${m.color}09`, border: `1px solid ${m.color}22`, backdropFilter: 'blur(8px)' }}>
              ...
              <div className="text-[9px]" style={{ color: m.color, fontFamily: 'monospace' }}>{m.sub}</div>
            </motion.div>
          ))}
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.82 }}
          ...
          {SOCIAL_PROOF.map((sp, i) => (
            <div key={i} className="flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full"
              style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', color: '#8fa3bf' }}>
              {sp.icon}
              <span>{sp.text}</span>
            </div>
          ))}
